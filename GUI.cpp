                            ...
                            if (action == "gather"){
                                ...
                                for (auto& p : players) {
                                    if (p.get() != current && dynamic_cast<Governor*>(p.get()) && p.get()->getInGame()) {
                                        governors.push_back(p.get());
                                    }
                                }
                                if (!governors.empty()) {
                                    pendingUndoResponders = governors;
                                    undoResponder = pendingUndoResponders.front();
                                    lastActor = current;
                                    screen = UNDO_PROMPT;
                                    continue;
                                ...
                                for (auto& p : players) {
                                    if (p.get() != current && dynamic_cast<Judge*>(p.get()) && p.get()->getInGame()) {
                                        judges.push_back(p.get());
                                    }
                                }
                                if (!judges.empty()) {
                                    pendingUndoResponders = judges;
                                    undoResponder = pendingUndoResponders.front();
                                    lastActor = current;
                                    screen = UNDO_PROMPT;
                                    continue;
